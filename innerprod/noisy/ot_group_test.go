...
func TestSimpleOT(t *testing.T) {
	...
	mod := new(big.Int).Add(new(big.Int).Mul(boundX, new(big.Int).Mul(boundX, big.NewInt((int64)(numClient)*(int64)(vecLen)))), new(big.Int).Add(big.NewInt(1), boundN))

	// build the scheme
	fe := noisy.NewOT(numClient, vecLen, boundX, boundN)

	// generate master secret key
	masterSecKey, err := fe.GenerateKeys()
	if err != nil {
	...
	for i := 0; i < numClient; i++ {
		cipher[i], err = fe.Encrypt(x[i], masterSecKey[i])
		if err != nil {
	...
	key, err := fe.DeriveKey(masterSecKey, y, noise)
	if err != nil {
		fmt.Printf("Error during derive key: %v", err)
	}

	// simulate a decryptor
	decryptor := noisy.NewOTFromParams(fe.Params)
