...
func TestNH_Quad_Adap(t *testing.T) {
	...
	// build the scheme
	fe := noisy.NewNHQuadAdap(secLevel, numClient, vecLen, new(big.Int).Mul(boundX, boundX), boundY, boundN)
	...
	cipher := make([]*noisy.NHQuadAdapCT, numClient)
	for i := 0; i < numClient; i++ {
		cipher[i], err = fe.Enc(enckeys[i], i, x[i])
		if err != nil {
	...
	decryptor := noisy.NewNHQuadAdapFromParams(fe.Params)
