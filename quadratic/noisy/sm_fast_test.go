...
func TestSM_Fast(t *testing.T) {
	...
	fmt.Printf("***********New Idea: clients: %d, veclen: %d*************\n", numClient, vecLen)

	// build the scheme
	start := time.Now()
	fe := noisy.NewOTQuad(secLevel, numClient, vecLen, boundX, boundY, boundN)
	...
	// encrypt vectors
	start = time.Now()
	cipher := make([]*noisy.OTQuadCT, numClient)
	for i := 0; i < numClient; i++ {
